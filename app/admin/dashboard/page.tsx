...
const PERIOD_OPTIONS = [
  ...
  { value: "bulan", label: "30 Hari Terakhir" },
  { value: "tahun", label: "12 Bulan Terakhir" },
];
                  ...
                  {data.paymentData.map(p => (
                    <div key={p.name} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100">
                      <div className="flex items-center gap-2">
                        <div className="w-3 h-3 rounded-full" style={{ background: PAYMENT_COLORS[p.name] || "#94a3b8" }} />
                        <span className="font-semibold text-slate-700 text-base">{p.name}</span>
                      </div>
                      <span className="font-bold text-slate-800 text-base">Rp {p.value.toLocaleString("id-ID")}</span>
                    </div>
                  ))}
                </div>
              </div>
